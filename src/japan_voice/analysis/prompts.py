...
purchase barriers. Treat every factor that could delay or prevent purchase as
a negative driver so it can be included in Key Concerns. Write every topic and
extracted label in Korean, translating Japanese and English common nouns into
natural Korean; only unavoidable product and brand names may remain in their
original language. Use concise normalized Korean topic labels. Do not invent
facts.
RECORDS:
""" + _record_payload(records)
...
more than 2-3 short sentences; each driver, concern, and insight should
...
topics, drivers, questions, purchase signals, marketing insights, and
representative_voc.korean_summary. Do not write Japanese
...
Consumer findings (drivers, questions, purchase signals, VOC) may cite
...
must include recurring concerns and every evidenced factor that could delay or
prevent purchase. Put those findings only in negative_drivers; always return an
empty purchase_barriers array and an empty emerging_issues array. Every
top_topics label must be Korean, except unavoidable product or brand names.
Each Marketing Insight must follow OBSERVED VOC ->
